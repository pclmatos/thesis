...
Para fazer citações, deverá usar-se a chave da referência no ficheiro BibTeX. Se for uma única referência, usar um ``\verb!~!'' para ligar o \verb!\cite{...}! à palavra que o precede (\ldots\verb!referência!).  Caso queira fazer múltiplas citações, deverá agrupá-las dentro de um úinico \verb!\cite{...}!.
