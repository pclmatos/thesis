...
%\textit{Scribe}~\cite{scribe} is an application-layer multicast infrastructure built on top of \textit{Pastry}, a self-organizing overlay network of nodes. \textit{Scribe} nodes can create and join multicast groups to propagate messages to other nodes. This protocol provides a best-effort delivery of messages, and specifies no particular delivery order by relying on TCP connections to reliably disseminate messages. Some systems might require stronger reliability guarantees, therefore, Scribe provides the framework for developers to implement stronger reliability guarantees.

%\subsubsection{P2S}

%\textit{P2S}~\cite{p2s} is a crash-tolerant, topic-based pub/sub middleware, that replicates the broker using \textit{Goxos}, a Go implementation of \textit{Paxos}. \textit{Paxos} is a fault-tolerant consensus protocol, in which replicas try to reach agreement on a value. This protocol, provides strong consistency because replicas only agree on a value in case at least, \textit{f}+1 replicas accept a value proposed by another replica, where \textit{f} is the number of faults that can be tolerated. A big advantage of \textit{Paxos} is that if at least \textit{f} replicas fail, it will not make progress since it can not reach consensus on a value and, therefore, it ensures strong consistency among replicas. In \textit{P2S}, client requests are treated as \textit{Paxos} proposals, thus every request is disseminated to every other replica to achieve consensus. After reaching consensus, brokers can forward the messages to all subscribers subscribed to the message topic. To address broker failures, \textit{P2S} relies on a set of 2\textit{f}+1 replicated brokers between publishers and subscribers, where up to \textit{f} brokers can fail.
